...
{
	memset(&(p_info->mfptp_info),0,sizeof(p_info->mfptp_info));
...
int got_frame_data_ev(struct user_info* usr, char * buf, int len)
{
	...
	if ( (len - offset) >= remain){
		...
		x_printf(D,"frame 数据不全，继续接收\n");
		memcpy(p_status->frame_data + p_status->frame_offset,buf + offset, (len - offset));	
		p_status->recv_buf_index += (len - offset);
		p_status->frame_offset += (len - offset);
		x_printf(D,"recv_buf_index = %d  frame offset = %d\n",p_status->recv_buf_index,p_status->frame_offset);
...
void mfptp_parse_FP_size(struct user_info *usr){
	...
	unsigned char *ptr = p_status->FP_size;
...
void mfptp_parse_frame(struct user_info *usr){
	...
	p_status->package.ptrs[p_status->package.frames - 1] = p_status->frame_data;
       ...
       // x_printf(D, "第一帧地址=%p,帧长度=%d\n",p_status->package.ptrs[0],p_status->package.dsizes[0]);
    x_printf(D, "帧地址=%p,帧长度=%d\n",p_status->package.ptrs[p_status->package.frames-1],p_status->package.dsizes[p_status->package.frames - 1]);
		
#if 0 
	//	x_printf(D,"第%d帧数据,地址%p\n",p_status->package.frames,p_status->package.ptrs[p_status->package.frames - 1]);
		int index = 0;
	...
	p_status->frame_offset = 0;// 一帧完成以后，需要reset
...
void  mfptp_parse_package(struct user_info* usr)
{
	...
	// 释放内存，链表循环释放
    for (i = 0; i < p_status->package.frames; i++){
		x_printf(D,"p_status->package.ptrs[%d] = %p\n",i,p_status->package.ptrs[i]);
...
int mfptp_parse_fsm(struct user_info* usr, char * buf, int len)
{
    ...
    for( ; ;)	
	{
		switch(p_status->state){
			case INIT_STATE:
				//在header前，把recv_len
				//设置为0,这个长度在握手的时候需要传给客户端 续传是对每一次请求而言
				p_status->recv_len = 0;
			...
			case GOT_FRAME_DATA_STATE:
				// 先写下代码，然后再看看能抽象不
				// 1. 先判断是否是一个包
				x_printf(D,"complete :%d\n",p_status->package.complete);
...
//        x_printf(D, "帧地址=%p,帧长度=%d\n",p_status->package.ptrs[0],p_status->package.dsizes[0]);
					...
					if (p_status->package_index == p_parser->packages){
						x_printf(D,"一共%d包,现在第%d包\n",p_parser->packages,p_status->package_index);
						move_to_state(usr,OVER_STATE);
						p_status->package_index = 0;//一次请求结束，包index reset to 0
					}else {
						// 下一帧
						move_to_state(usr,GOT_HEADER_STATE);
					}
